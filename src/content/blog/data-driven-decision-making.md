...
threeLevelNote: |
  - L1 用数据支撑决策的前提是“测量值得做”
    - L2 在埋点前先回答四个决策问题
      - L3 预期结果与原因：暴露先验假设，减少分析偏见
      - L3 正向结果行动：若支持判断要有明确后续动作
      - L3 反向结果行动：若不支持应调整或停止方案，而非找补
      - L3 决策责任与时点：明确谁在何时根据数据做决定
    - L2 指标 ROI 来自“会改变行动”的数据
      - L3 若无行动计划或不会改变决策，追踪该指标没有价值
  - L1 GSM 框架把“目标—信号—指标”建立可追溯链路
    - L2 Goals 定义期望结果并先达成共识
      - L3 通过相互制衡目标避免优化单一指标导致副作用
    - L2 Signals 描述判断目标是否达成的现象
      - L3 信号可量化也可不可量化；不可量化信号可暴露监控盲区
      - L3 目标与信号非一对一：一个目标至少一个信号，多个目标可共享信号
    - L2 Metrics 用可测数据反映信号
      - L3 指标可定量或定性，并可用多指标交叉验证信号
    - L2 GSM 的方法价值
      - L3 避免“路灯效应”：不只看容易拿到的数据
      - L3 指标选取逻辑可解释、可质疑、可达成团队共识
      - L3 每个 Metric 可反推其 Signal 与 Goal，增强可追溯性
  - L1 购物车“一键清理失效商品”案例展示了框架落地
    - L2 先验证方案是否值得测量
      - L3 假设：功能应提升含失效商品购物车会话的结算转化率
      - L3 行动阈值：两周灰度后由产品经理决定是否全量
    - L2 设计双目标与对应指标
      - L3 业务结果目标：提升进入结算页转化率
      - L3 风险约束目标：误操作与相关投诉不明显增加
      - L3 诊断指标：按钮点击率、清理后停留率仅用于解释原因
    - L2 防止“漂亮局部指标”误导结论
      - L3 即使点击率高，若核心转化率不升，方案仍不成立
      - L3 数据应能推翻原假设，而不是为既定结论找证据
---
